...
func TestTrafficManagement_ServiceResolverSubsetOnlyPassing(t *testing.T) {
	...
	run := func(t *testing.T, tc testcase) {
		...
		services, _, err := cluster.Agents[0].GetClient().Health().Service("static-server", "", true, nil)
		require.NoError(t, err)
		...
		// switch back to OnlyPassing=false by deleting the config
		...
		// static-client upstream should have 2 healthy endpoint for static-server
